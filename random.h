...
_NODISCARD _Uint _Next_linear_congruential_value(_Uint _Prev) noexcept {
    ...
    if _CONSTEXPR_IF(_Ax == 0) { // degenerate case; avoid divide by 0
        return static_cast<_Uint>(_Cx); // relies on _Mx == 0 || _Cx <= _Mx, N4741 [rand.eng.lcong]/3
    }
    else if _CONSTEXPR_IF(_Mx == 0) {
    ...
    else if _CONSTEXPR_IF(_Cx <= UINT_MAX && static_cast<_Uint>(_Mx - 1) <= (UINT_MAX - _Cx) / _Ax) {
    ...
    else if _CONSTEXPR_IF(_Cx <= ULLONG_MAX && static_cast<_Uint>(_Mx - 1) <= (ULLONG_MAX - _Cx) / _Ax) {
...
_NODISCARD _Uint _Get_linear_congruential_seed_from_seq(_Seed_seq& _Seq) { // N4741 [rand.eng.lcong]/6
    _Uint _Sx;
    if _CONSTEXPR_IF(_Mx == 0) {
        if _CONSTEXPR_IF(sizeof(_Uint) <= sizeof(unsigned int)) {
    ...
    else if _CONSTEXPR_IF(_Mx <= UINT_MAX) {
...
public:
    ...
    void seed(_Seed_seq& _Seq) { // reset sequence from seed sequence
        ...
        for (size_t _Ix = 0; _Ix < _Rx; ++_Ix, _Idx0 += _Kx) { // pack _Kx words
            ...
            if _CONSTEXPR_IF(_Mod_non_zero) {
